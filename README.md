...
HappyLocaleは、カード画像やPDFに日本語訳を配置し、画像や印刷用PDFとして書き出せる編集ツールです。
OCRで原文を読み取り、CSVで訳文をまとめて編集できます。
画像編集・OCR・PDF解析はブラウザ内で行い、カードの編集データはPCのフォルダに保存できます。
...
- ルールPDFの文字をCSVへ出し、訳文を重ねた別PDFを作る
- アイコンやフォントをローカルで読み込み、カード間で再利用する

## 画面例

トップページの「サンプル（デモ）を開く」から、自作のサンプルカード5枚で領域検出・翻訳確認・印刷を試せます。デモではプロジェクト保存はできません。素材の説明は[サンプルカード](samples/cards/README.md)、画面例は[翻訳確認画面](docs/translation-review-prototype.md)を参照してください。
...
- **ルールPDFに訳文を重ねる**：PDF内の文字情報を取り出してCSVで訳文を編集し、原文部分を覆って日本語訳を配置したPDFを作れます。
...
- Translation Endpoint設定: ブラウザの`localStorage`
...
3. 原文部分をドラッグして翻訳領域を作ります。
4. 背景処理を選び、原文と日本語訳、文字設定を編集します。
5. 「プロジェクト保存」で編集データと画像をローカルフォルダへ保存します。
6. カードごとのPNG／JPEG、翻訳CSV、または印刷範囲を面付けしたA4 PDFを書き出します。

最初にホームで「カード編集」または「PDF翻訳」を選びます。作業中はツールバー左端の作業切替メニューから直接切り替えられます。PDF翻訳はカード編集とは独立しており、プロジェクトフォルダを作らずに利用します。詳しい操作、CSV形式、アセット、フォント、PDFの制約は[利用ガイド](docs/user-guide.md)を参照してください。
...
- 画像全体のOCR領域候補は試験機能
- Translation Endpointはα機能。ローカル翻訳モックとの結合動作を確認済みで、Github Pages版では無効
